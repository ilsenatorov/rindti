...
from torch_geometric.nn import (GCNConv, GINConv, GraphSizeNorm,
                                JumpingKnowledge)
...
class GMTNet(BaseLayer):
    ...
    def __init__(self,
        ...
        self.sab = SAB(hidden_dim,
                       hidden_dim,
                       num_heads,
                       ln=ln,
                       cluster=cluster)
        self.gmpooli = PMA(output_dim,
                           num_heads,
                           1,
                           ln=ln,
                           cluster=cluster,
...
class PMA(LightningModule):
    ...
    def forward(self,
                ...
                graph=None,
                return_attn=False):
        return self.mab(self.S.repeat(X.size(0), 1, 1),
                        X,
                        attention_mask,
                        graph,
                        return_attn)
