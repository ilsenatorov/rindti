    ...
    def __init__(
        ...
        **kwargs,
        ...
        for i in range(num_layers - 2):
            self.mlp.add_module(f"hidden_linear{i}", nn.Linear(hidden_dim, hidden_dim))
            self.mlp.add_module(f"hidden_relu{i}", nn.ReLU())
            self.mlp.add_module(f"hidden_dropout{i}", nn.Dropout(dropout))
