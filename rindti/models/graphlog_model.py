...
class GraphLogModel(BaseModel):
    ...
    def update_proto_lowest(self, graph_reps, decay_ratio=0.7, epsilon=1e-6):
        ...
        self.proto[0] = (self.proto[0] * (cnt == 0).float().unsqueeze(-1) + (
            self.proto[0] * decay_ratio + batch_mean * (1 - decay_ratio)) * (cnt != 0).float().unsqueeze(-1)).detach()
    ...
    def init_proto(self, index, num_iter=20):
        proto_connection = torch.zeros(
            self.proto[index-1].shape[0], device=self.device)
        ...
        for iter in range(num_iter):
            for i in range(self.proto[index-1].shape[0]):
                ...
                self.proto[index][idx, :] = self.proto[index][idx,
                                                              :] * self.decay_ratio + self.proto[index-1][i, :] * (1 - self.decay_ratio)

                # penalize rival
                sim[idx] = 0
                rival_idx = torch.argmax(sim)
                self.proto[index][rival_idx, :] = self.proto[index][rival_idx,
                                                                    :] * (2 - self.decay_ratio) - self.proto[index-1][i, :] * (1 - self.decay_ratio)
    ...
    def shared_step(self, batch: Data, proto: bool = True) -> dict:
        ...
        NCE_inter_loss = self.inter_NCE_loss(
            graph_reps_proj, graph_modify_reps_proj)
        if proto:
    ...
    def training_epoch_end(self, outputs):
        if self.trainer.current_epoch == 0:
            self.proto = [torch.rand((self.num_proto, self.embed_dim), device=self.device)
    ...
    def configure_optimizers(self):
        ...
        '''
        optimiser = Adam(params=self.parameters(),
                         lr=self.hparams.lr,
